...
function App() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        let filtered = totalContestsList.filter(
          (obj) => obj.phase === contestPhase && obj.type === contestType
        );
  ...
  useEffect(() => {
    let filtered = totalContestsList.filter(
      (obj) => obj.type === contestType && obj.phase === contestPhase
    );
  ...
  useEffect(() => {
    setUiArray(
      totalContestsList
        .filter((obj) => obj.type === contestType && obj.phase === contestPhase)
        .slice(0, pageSize)
    );
  }, [pageSize]);
  ...
  useEffect(() => {
    ...
    let filtered = totalContestsList
      .filter((obj) => obj.type === contestType && obj.phase === contestPhase)
      .slice((pageNo - 1) * pageSize, pageSize * pageNo);
    setUiArray(filtered);
  }, [pageNo]);
    ...
    <div className="App">
      ...
      <div>
          ...
          <div className="app-page">
            {uiArray.map((obj) => (
              <div className="app-page-details" key={obj.id}>
                <h3>{obj.name}</h3>
                <p>
                  Duration : {convertSectoMins(obj.durationSeconds)} Minutes
                </p>
        ...
        <footer className="app-page-pagination">
          ...
          <button className="btn-navigate" onClick={previousPage}>
            Previous
          </button>
          <button className="btn-navigate" onClick={nextPage}>
            Next
          </button>
        </footer>
      </div>
